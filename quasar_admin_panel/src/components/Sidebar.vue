<template>
  <nav id="sidebar" class="sidebar" :class="{collapsed: !isOpenSidebar}">
    ...
    <ul class="sidebar-nav">
      ...
      <li v-for="route in this.$router.getRoutes().filter(route => route.meta.isMenu)" :key="route.name" v-bind:class="{ active: $route.path === route.path }" class="sidebar-item">
        <router-link :to="route.path" class="sidebar-link">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" v-html="route.meta.svg"></svg>
