    ...
    <div class="card">
      <div class="card-body">
        <div class="card-container">
          <div class="text-center">
            <img
              src="../assets/img/quasar_logo.png"
              alt="Logo"
              class="card-img"
              width="150"
              height="150"
          ...
          <form>
            ...
            <div class="password-field">
              ...
              <input
                v-model="this.passwordInputValue"
                class="form-control form-control-lg"
                type="password"
                name="password"
            ...
            <div
              class="card-error"
              :class="{ 'login-error': this.cardErrorIsShown }"
            >
            ...
            <div class="card-button">
              <a
                @click.prevent="validationButtonClick"
...
export default {
  ...
  methods: {
    validationButtonClick() {
      // функция для обработки события нажатия на кнопку валидации
      if (this.validationForm()) {
        // если форма валидна, вызываем функцию авторизации
        this.authorization();
      }
    },
    validationForm() {
      ...
      if (this.emailInputIsValid && this.passwordInputIsValid) {
        // если оба поля валидны, сбрасываем флаг ошибки и возвращаем true
        this.cardErrorIsShown = false;
        return true;
      } else {
        // если есть ошибки, устанавливаем флаг ошибки и возвращаем false
        this.cardErrorIsShown = true;
        return false;
      }
    },
    emailRules(value) {
      // функция для проверки валидности ввода почты
      console.log(value);
      const pattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (value && pattern.test(value)) {
        // если введенная почта валидна, устанавливаем флаг валидности почты и возвращаем true
        this.emailInputIsValid = true;
        console.log("Email is valid");
        return true;
      } else {
        // если введенная почта не валидна, сбрасываем флаг валидности почты и возвращаем false
    ...
    passwordRules(value) {
      // функция для проверки валидности ввода пароля
      console.log(value);
      if (value && value.length >= 8) {
        // если введенный пароль валиден, устанавливаем флаг валидности пароля и возвращаем true
        this.passwordInputIsValid = true;
        console.log("Password is valid");
        return true;
      } else {
        // если введенный пароль не валиден, сбрасываем флаг валидности пароля и возвращаем false
    ...
    async authorization() {
      ...
      const result = await db.authorizationUser(
      ...
      if (result !== false) {
        ...
        // Если авторизация не прошла успешно, показываем ошибку в форме
